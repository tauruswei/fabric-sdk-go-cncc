...
import (
	...
	"errors"
	"fmt"
	factory "github.com/hyperledger/fabric-sdk-go/third_party/github.com/hyperledger/fabric/bccsp/factory/cncc"
	"github.com/hyperledger/fabric-sdk-go/third_party/github.com/hyperledger/fabric/bccsp/verifier"
	"io"
	"math/big"
	...
	//"github.com/hyperledger/fabric-sdk-go/internal/github.com/hyperledger/fabric/bccsp/verifier"
...
func (ka *ecdheKeyAgreement) generateServerKeyExchange(config *Config, cert *Certificate, clientHello *clientHelloMsg, hello *serverHelloMsg) (*serverKeyExchangeMsg, error) {
	...
	bccsp, err := factory.GetBCCSP("CNCC_GM")
	if err != nil {
		fmt.Println("ServerHandshake error: %s", err.Error())
		return nil, err
	}
	verifier, err := verifier.New(bccsp, nil)
	if err != nil {
		return nil, err
	}
...
NextCandidate:
	...
	digest, hashFunc, err := hashForServerKeyExchange(sigAndHash, ka.version, clientHello.random, hello.random,
		serverECDHParams)
	if err != nil {
		return nil, err
	}
	...
	if x509Cert.SubjectKeyId == nil {
		sig, err = priv.Sign(config.rand(), digest, hashFunc)
	} else {
		sig, err = config.Verifier.Sign(x509Cert.SubjectKeyId, digest)
		//fmt.Println("=========================================")
		//fmt.Println("签名  generateServerKeyExchange")
...
func (ka *ecdheKeyAgreement) processServerKeyExchange(config *Config, clientHello *clientHelloMsg, serverHello *serverHelloMsg, cert *sm2.Certificate, skx *serverKeyExchangeMsg) error {
	...
	case signatureECDSA:
		...
		switch pubKey.Curve {
		case sm2.P256Sm2():
			//fmt.Println("验证签名 sm2.P256Sm2()")
			if config.Verifier != nil {
				//fmt.Println("=================================")
				//fmt.Println("验签:processServerKeyExchange")
				//fmt.Println(cert.Subject.CommonName)
				//fmt.Println("=================================")

				_, err := config.Verifier.Verify(cert.SubjectKeyId, sig, digest)
				if err != nil {
